...
async function runSampleData() {
    ...
    const startRoundAndVote = async () => {
        ...
        for (const candidate of candidates) {
            ...
            const isCandidateEliminated = await roundsBase.isEliminated(candidate.address);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        await network.provider.send("evm_increaseTime", [settings.roundDuration + 1]);
